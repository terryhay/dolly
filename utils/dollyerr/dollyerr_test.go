...
func TestDollyError(t *testing.T) {
	...
	t.Run("getters", func(t *testing.T) {
		...
		require.Equal(t, err, dollyErr.Error())
	})
...
func TestNewErrorIfItIs(t *testing.T) {
	...
	err := fmt.Errorf(gofakeit.Name())
	require.Equal(t, fmt.Errorf("%s: %s", prefix, err.Error()), NewErrorIfItIs(CodeUndefinedError, prefix, err).Error())
}
