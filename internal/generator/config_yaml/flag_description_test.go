...
func TestFlagDescriptionUnmarshalErrors(t *testing.T) {
	...
	testData := []struct {
	...
	}{
		{
			...
			expectedErrorText: "config_yaml.GetConfig: unmarshal error: flagDescription unmarshal error: no required field \"flag\"",
		},
		{
			yamlFileName:      "no_description_help_info.yaml",
			expectedErrorText: "config_yaml.GetConfig: unmarshal error: flagDescription unmarshal error: no required field \"description_help_info\"",
		},
	}
