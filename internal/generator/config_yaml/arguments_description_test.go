...
func TestArgumentsDescriptionGetters(t *testing.T) {
	...
	t.Run("nil_pointer", func(t *testing.T) {
		require.Equal(t, dollyconf.ArgAmountTypeNoArgs, pointer.GetAmountType())
...
func TestArgumentsDescriptionUnmarshalErrors(t *testing.T) {
	...
	testData := []struct {
	...
	}{
		{
			...
			expectedErrorText: "config_yaml.GetConfig: unmarshal error: argumentsDescriptions unmarshal error: no required field \"amount_type\"",
		},
		{
			yamlFileName:      "no_synopsis_description.yaml",
			expectedErrorText: "config_yaml.GetConfig: unmarshal error: argumentsDescriptions unmarshal error: no required field \"synopsis_description\"",
		},
		{
			yamlFileName:      "unexpected_amount_type.yaml",
			expectedErrorText: "config_yaml.GetConfig: unmarshal error: argumentsDescriptions unmarshal error: can't convert string value \"amount_type\": unexpected \"amount_type\" value: trash\\nallowed values: \"single\", \"array\"",
		},
	}
