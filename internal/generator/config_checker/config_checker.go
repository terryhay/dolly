...
func Check(
	...
	commandDescriptions map[string]*config_yaml.CommandDescription,
	flagDescriptions map[string]*config_yaml.FlagDescription,
) *dollyerr.Error {
...
func checkArgumentDescription(argDescription *config_yaml.ArgumentsDescription) *dollyerr.Error {
	...
	if defaultValuesCount == 1 {
		if argDescription.GetAmountType() != dollyconf.ArgAmountTypeSingle {
			return dollyerr.NewError(
		...
		if argDescription.GetAmountType() != dollyconf.ArgAmountTypeList {
			return dollyerr.NewError(
				dollyerr.CodeConfigUnexpectedDefaultValue,
